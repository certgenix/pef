import { db } from "./db";
import { users, userRoles, userProfiles, employerProfiles, opportunities } from "@shared/schema";
...
async function seedJobs() {
  ...
  let employerExists = await db.select().from(users).where(eq(users.id, sampleEmployerId)).limit(1);
  ...
  for (const job of jobs) {
    const existingJob = await db
      .select()
      .from(opportunities)
      .where(eq(opportunities.title, job.title))
      .limit(1);

    if (existingJob.length === 0) {
      await db.insert(opportunities).values(job);
